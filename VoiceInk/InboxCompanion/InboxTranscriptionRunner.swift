...
final class InboxTranscriptionRunner: InboxTranscriptionRunning {
    ...
    func run(request: InboxCompanionRequest, snapshot: InboxCompanionRuntimeSnapshot, cancellation: TranscriptionCancellationToken) async -> InboxCompanionResponse {
        ...
        defer {
            ...
            Task { @MainActor in await self.backend.cleanup() }
        }
        do {
            ...
            guard !text.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else { return failure(request, .emptyTranscript, "transcription", false) }
        ...
        } catch is CancellationError { return failure(request, .cancelled, "transcription", false) }
        catch let error as CloudTranscriptionError { return mapCloud(request, error) }
        catch let error as InboxCompanionPreflightError { return failure(request, error.failureCode, "preflight", false) }
        catch { return failure(request, .providerFailure, "transcription", true) }
    }
